var d3         = require("d3");
var flare_data = require("flare_data")
...
var listed;
var treeOption;
expFunc.drawTree = (_comp, _data, _option = {}) => {
  treeOption = _option;
  ...
  var editName = _comp + "-editme";
  var treeName = _comp + "-tree";

  var iDiv = document.createElement('div');
  iDiv.id = editName;
  iDiv.className = "editme"; 
  document.getElementById(_comp).appendChild(iDiv);
  ...
  var svg = d3.select("#" + treeName).append("svg")
  ...
  function start(flare) {
    ...
    var conved = util.make_listed(flare, undefined, true)
      , main = document.getElementById(editName)
...
function cleanTree(_d){
  var retTree = _.pick(_d, _(_.keys(_d)).filter(_k => {return ["name","value"].includes(_k)}).value());

  if(_.has(_d, "children")){
    retTree.children = [];
    _(_d.children).map(_e => retTree.children.push(cleanTree(_e))).value();
  }
  
  return retTree;
}

expFunc.dumpData = function(){
  return cleanTree( listed.ctrl.model.dumpData());
  //return listed.ctrl.model.dumpData();
}

expFunc.dumpOptionData = function(){
  return treeOption ;
}
